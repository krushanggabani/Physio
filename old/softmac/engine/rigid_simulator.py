...
class RigidSimulator:
    def __init__(self, cfg, primitives, substeps=20, env_dt=2e-3):
        ...
        print("About to construct World()…")
        w = nimble.simulation.World()                # ← parentheses are critical!
        print("World() ok.  Setting gravity…")
        w.setGravity(np.array([0.0, -9.81, 0.0]))    # try float64 or float32
        print("Gravity set.  Setting timestep…")
        w.setTimeStep(1e-3)
        print("All calls succeeded!  You're good to go.")
        ...
        for i, skeleton in enumerate(self.skeletons):
            ...
            self.nodeDepIdx += [
                np.array([skeleton.getBodyNode(j).getDependentGenCoordIndex(k)
                          for k in range(6) + start_idx])
    ...
    def add_floor(self):
        ...
        # floor_path = Path("assets/floor/floor.urdf").absolute()  # UPDATED: explicit floor path
        # floor = self.world.loadSkeleton(str(floor_path))
        # floor_node = floor.getBodyNode(0)
        # floor_node.setFrictionCoeff(1e3)
        # if floor is None:
        #     raise FileNotFoundError(f"Floor URDF not found or could not be loaded from {floor_path}")  # UPDATED: error handling
        # floor_node = floor.getBodyNode(0)
        # if floor_node is None:
        #     raise AttributeError("Loaded floor skeleton does not contain any body nodes.")  # UPDATED: error handling
        # floor_node.setFrictionCoeff(1e3)
        return floor
    ...
    def step(self, s, action=None):
        ...
        for i in range(self.n_primitive):
            ...
            if (ext_f.abs() > 1e-10).any() and self.primitives[i].enable_external_force:
                ...
                jacob_ds_df = np.array([0,0,0,0,0,0])
