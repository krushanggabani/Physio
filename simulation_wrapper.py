...
# ─── Simulation globals (will be overwritten in run_simulation) ─────────────
dim         = 2
n_particles = 20000
n_grid      = 128
...
k1, k2       = 1.0, 1.0   # spring stiffness
...
def init_mpm():
    for p in range(n_particles):
        ...
        theta = ti.random() * PI
        x[p]  = ti.Vector([soft_center_x + r * ti.cos(theta), floor_level + r * ti.sin(theta)])
        v[p]  = [0.0, 0.0]
...
def update_base_and_arm():
    ...
    time_t += dt * 15
    base_y = y0 + A * np.cos(omega * time_t)
    ...
    alpha1 = tau1 / I1
    alpha2 = tau2 / I2
    dtheta1[0] += alpha1 * dt; theta1[0] += dtheta1[0] * dt
    dtheta2[0] += alpha2 * dt; theta2[0] += dtheta2[0] * dt

# Wrapper

def run_simulation(E_val, nu_val, rho_val, mu_floor_val, cycles: int):
    global E, nu, p_rho, p_mass, mu_0, lambda_0, floor_friction
    # Update material & contact params
    E, nu = E_val, nu_val
    p_rho = rho_val
    p_mass = p_rho * p_vol
    mu_0 = E / (2 * (1 + nu))
    lambda_0 = E * nu / ((1 + nu) * (1 - 2 * nu))
    floor_friction = mu_floor_val

    # Initialize sim
    init_mpm()
    initial_x = x.to_numpy().copy()
    deformations = []
    forces = [] 
    gui = ti.GUI('MPM + Single Passive Arm (Attached & Outward)', res=(512, 512))

    steps_per_cycle = 15  # same as main GUI loop
    for _ in range(100):
        previous_x = x.to_numpy()
        for __ in range(steps_per_cycle):
            p2g()
            apply_grid_forces_and_detect()
            g2p()
            update_base_and_arm()
        curr_x = x.to_numpy()
        disp = np.linalg.norm(curr_x - previous_x, axis=1)
        deformations.append(float(disp.max()))
        Fc = contact_force_vec[0].to_numpy()
    ...
    return np.array(deformations, dtype=np.float32), np.array(forces, dtype=np.float32)
